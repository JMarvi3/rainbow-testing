...
uint16_t read_big_uint16(void *buff, ssize_t *pos) {
    uint16_t val = ntohs(*(uint16_t *)(buff + *pos));
    *pos += 2;
    return val;
}

uint32_t read_big_uint32(void *buff, ssize_t *pos) {
    uint32_t val = ntohl(*(uint32_t *)(buff + *pos));
    *pos += 4;
    return val;
}

uint16_t read_little_uint16(void *buff, ssize_t *pos) {
    uint16_t val = *(uint16_t *)(buff + *pos);
    *pos += 2;
    return val;
}

uint32_t read_little_uint32(void *buff, ssize_t *pos) {
    uint32_t val = *(uint32_t *)(buff + *pos);
    *pos += 4;
    return val;
}

int16_t read_little_int16(void *buff, ssize_t *pos) {
    int16_t val = *(int16_t *)(buff + *pos);
    *pos += 2;
    return val;
}

int32_t read_little_int32(void *buff, ssize_t *pos) {
...
PyObject *py_decode_ms(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    ssize_t pos = data_offset;
    ssize_t start_pos = read_big_uint16(file, &pos) * 2 - 2;
    pos = start_pos;
